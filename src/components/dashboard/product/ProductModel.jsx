...
import { deleteData, postData } from '../../api/apiCalls'
...
font-weight: 100;
`
const ProductModel= ({setOpen,product:tempProduct,action}) => {
  ...
  const onSubmit = async e =>{
    ...
    try {
      ...
      const res = action === 'delete'? await deleteData(`/products/${product?._id}`) : await postData(`/products/${product?._id}`,product);
      if(res){
        console.log(res);
        const newRes = await postData(`/activity?type=PRODUCT_UPDATE`);
        console.log(newRes);
  ...
  return (
      <Container>
        ...
        <Title> {action ==='delete'?'Confrim Delete':'Update Stock'} </Title>
        <WrapperTwo>
      <Form onSubmit ={onSubmit} >
        ...
        <ControlWrapper>
            <Control className='active' disabled = {disabled} > {loading ? <LoadingAnimation/>:action === 'delele '? 'DELETE PRODUCT':'UPDATE PRODUCT'} </Control>
        </ControlWrapper>
      </Form>
    </WrapperTwo>
      </Container>
